...
Event devices have an additional control in the `controls` array for the Event & Tally REST API (see [Event & Tally REST API](6.0._Event_and_tally_rest_api.md)).
...
The `ext_is_07_rest_api_url` parameter represents the url to the API path which offers the current state and type of an event emitter (source) (see [Event & Tally REST API](6.0._Event_and_tally_rest_api.md))
